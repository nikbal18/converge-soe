...
Usage:
    python cim_to_json.py feeder.xml -o network.json
    python cim_to_json.py feeder.xml -o network.json --loads loads.csv
    python cim_to_json.py feeder.xml --lv-vmin 0.39 --lv-vmax 0.44
"""
...
def convert(xml_paths, output=None, loads=None, no_placeholder_loads=False,
            v_setpoint_pu=1.05, lv_vmin=None, lv_vmax=None,
            lv_voltage_tolerance=0.10, lv_nominal_v=None, source_node=None,
            keep_nmi_checksum=False):
    """Importable API: convert CIM XML file(s) to an ejson dict.

    Returns (ejson, stats, warnings). If ``output`` is given the ejson is
    also written there.
    """
    args = argparse.Namespace(
        xml=list(xml_paths), output=output, loads=loads,
        no_placeholder_loads=no_placeholder_loads, v_setpoint_pu=v_setpoint_pu,
        lv_vmin=lv_vmin, lv_vmax=lv_vmax,
        lv_voltage_tolerance=lv_voltage_tolerance, lv_nominal_v=lv_nominal_v,
        source_node=source_node, keep_nmi_checksum=keep_nmi_checksum,
    )
